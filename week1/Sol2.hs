...
-- VVZ: it is spelled "contradiction"
...
-- VVZ: do you use the fact that the filtered array contains p(x)? Nope, you only use the length, hence we can write it without map:
-- VVZ: unique p xs = length (filter p xs) == 1

-- > Ex 2.52
--TRUE just in case that an even number of the xs's equals TRUE
-- VVZ: wha? I understand the function, but I don't understand the description above
parity :: [Bool] -> Bool
...
-- VVZ: why not just 'evenNR = parity . map' ?
