...
-- VVZ: technically, it is not a print, which intuitively should have a type in the lines of String -> IO ()
...
-- VVZ: the above line kinda works, but is not optimal since in this algorithm you compute the minimum twice.
-- VVZ: you can use the 'where' clause in Haskell, something like:
-- VVZ: srtString xs = x : (srtString  (rmStrFrmLst x xs)) where x = minimum xs
...
-- VVZ: why an error? You are asked to measure lengths of 0 things, the result is just an empty array
lengths (x:xs) = map length (x:xs)
-- VVZ: ...which happens automatically with map.length ;)
